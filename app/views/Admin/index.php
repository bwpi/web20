<div class="h3 text-center" id="title">Расписание занятий в кабинете <?=$cabinet?>, режим просмотра</div>
<div class="table-responsive-xl">
    <table class="table table-hover table-bordered table-dark shadow-lg" id="schedule">
        <thead>
            <tr>
                <th scope="col">#</th>
                <?php foreach ($data as $key_day_ru => $sched):?>
        ...
        <tbody>
            ...
            <tr>
                <th scope="row"><?=$i;?></th>
                <?php foreach ($data as $key_day_ru => $schedule):?>
                <?php $text = $data[$key_day_ru][$i];?>
                <td class="p-0" data-day="<?="$key_day_ru/$i";?>">
                    <?php if (!empty($text)):?>
